...
import indi.augusttheodor.aurora.adapter.AdminReportArticleAdapter;
...
public class AdminReportByHotActivity extends AppCompatActivity implements RecyclerViewListenerInterface , HttpInterface {
    //说实话这种类也挺有代表性的，要不封装下？
    //主要是我Java学的没那么好，关于父类和子类的认知也就那样，不然我真封装了。

    public RecyclerView recycler;
    public AdminReportArticleAdapter adapter;
    public AdapterItems<AdminReportArticleViewHolder.AdminReportArticleObject> items;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView)findViewById(R.id.title)).setText(getString(R.string.manage_report_by_hot));
        this.recycler=findViewById(R.id.recycler);
        this.items=new AdapterItems<>();
        this.adapter=new AdminReportArticleAdapter(this.items,this);
    ...
    public void onNext() {
        AuroraApplication.httpPackage.asyncRequest(
                getString(R.string.backend_website)+getString(R.string.backend_report_article).replace("0",group_id)+p,
                "GET",
                null,
                "REPORT_ARTICLE",
                this,
        ...
        switch (tag){
            case "REPORT_ARTICLE":
                if(response.code()==200){
                    ...
                    List<AdminReportArticleViewHolder.AdminReportArticleObject> ra=AuroraApplication.httpPackage.gson.fromJson(s,new TypeToken<List<AdminReportArticleViewHolder.AdminReportArticleObject>>(){}.getType());
                    for(AdminReportArticleViewHolder.AdminReportArticleObject rao:ra){
                        this.items.add(rao);
                    }
